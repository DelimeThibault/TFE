...
def on_message(client, userdata, msg):
    ...
    ts_approx_ms = int(time.time() * 1000)

    if msg.topic == TOPIC_REALTIME:
        payload["ts_approx_ms"] = ts_approx_ms
    ...
    if message_handler:
        message_handler(event_name, payload, state)
...
def publish_timebase():
    ...
    while True:
        try:
            timebase_seq += 1
            payload = {"ts_app_ms": int(time.time() * 1000), "seq": timebase_seq}
